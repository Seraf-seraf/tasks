...
REST API сервис для управления задачами в командах. Сервис поддерживает регистрацию и JWT-аутентификацию, роли в командах, историю изменений задач, комментарии, Redis-кеширование списков задач и отчетные SQL-запросы.
...
Для подключения к другой базе передайте `DB_DSN`:

```bash
make db-up DB_DSN='user:pass@tcp(localhost:3306)/tasks?parseTime=true&multiStatements=true'
```
